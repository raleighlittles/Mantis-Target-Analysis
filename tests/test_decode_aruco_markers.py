...
class TestDecodeArucoMarkers(unittest.TestCase):
    def test_gather_images_directory_filters_extensions_and_sorts(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            (tmp_path / "b_image.jpg").write_bytes(b"")
            (tmp_path / "a_image.png").write_bytes(b"")
            ...
            self.assertEqual(
                images,
                [tmp_path / "a_image.png", tmp_path / "b_image.jpg"],
            )
